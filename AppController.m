...
- (void)createMenuBar
{
  ...
  // Current status menu item
//  NSString *currentStatus;
//  bool _inRange = [self isInRange];
//  if (_inRange == TRUE)
//    currentStatus = @"Status: Connected";
//  else
//    currentStatus = @"Status: Disconnected";
//  [myMenu addItemWithTitle:currentStatus action:@selector(terminate:) keyEquivalent:@""];
...
- (void)handleTimer:(NSTimer *)theTimer
{
	if( [self isInRange] )
	{
      ...
      [self showInRangeNotification:nil];
	...
	else
	{
		if( priorStatus == InRange )
		{
			...
			[self menuIconOutOfRange];
      [self showOutRangeNotification:nil];
...
- (void)userDefaultsLoad
{
  ...
  if (notifications)
  {
    [enableNotifications setState:NSOnState];
    notify = true;
  }
...
- (void)userDefaultsSave
{
  ...
  BOOL notifications = ( [enableNotifications state] == NSOnState ? TRUE : FALSE );
  [defaults setBool:notifications forKey:@"notifications"];
...
- (IBAction)showInRangeNotification:(id)sender{
  if (notify)
  {
    ...
    notification.title = @"Proximity";
    notification.informativeText = @"Device came into range";
...
- (IBAction)showOutRangeNotification:(id)sender{
  if (notify)
  {
    NSUserNotification *notification = [[NSUserNotification alloc] init];
    notification.title = @"Proximity";
    notification.informativeText = @"Device went out of range";
    notification.soundName = NSUserNotificationDefaultSoundName;
